						...
						<tbody>
							<c:forEach items="${studentList }" var="student">
								<tr class="gradeA">
									...
									<td>
										<c:if test="${student.user.state eq '0' }">正常</c:if>
										<c:if test="${student.user.state eq '1' }">冻结</c:if>
									</td>
									<c:if test="${sessionScope.login_user.type eq '2' }">
										<td>
											<a href="${ctx }/studentManager/freeze.do?id=${student.user.id}">冻结</a>
											&nbsp;
											<a href="${ctx }/studentManager/unFreeze.do?id=${student.user.id}">解冻</a>
											&nbsp;
											<a href="${ctx }/studentManager/delete.do?id=${student.user.id}">删除</a>
										</td>
									</c:if>
								</tr>
							</c:forEach>
